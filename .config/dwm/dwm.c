...
static void viewneighbor(const Arg *arg);
...
void viewneighbor(const Arg *arg) {
  unsigned int newtags = 0;
  int i = __builtin_ctz(selmon->tagset[selmon->seltags]);

  if (arg->i > 0) { //(pgup)
    if (i < LENGTH(tags) - 1)
      newtags = 1 << (i + 1);
    else
      newtags = 1; // wrap to tag 0
  } else {
    if (i > 0)
      newtags = 1 << (i - 1);
    else
      newtags = 1 << (LENGTH(tags) - 1); // wrap to last
  }
